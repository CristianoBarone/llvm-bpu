//===-- BPUISelLowering.cpp - BPU DAG Lowering Implementation -----------===//
//
...
#define DEBUG_TYPE "bpu-isellower"

#include "BPUGenCallingConv.inc"

BPUTargetLowering::BPUTargetLowering(const TargetMachine &TM,
                                         const BPUSubtarget &STI)
    : TargetLowering(TM), Subtarget(STI)
{
  // Set up the register classes
  addRegisterClass(MVT::i32, &BPU::GPRRegClass);
  ...
  setStackPointerRegisterToSaveRestore(BPU::X2);
...
const char *BPUTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case BPUISD::Ret: return "BPUISD::Ret";
  default:            return NULL;
  }
}

void BPUTargetLowering::ReplaceNodeResults(SDNode *N,
...
static const MCPhysReg GPRArgRegs[] = {
  BPU::X0, BPU::X1, BPU::X2, BPU::X3
};

/// LowerFormalArguments - transform physical registers into virtual registers
/// and generate load operations for arguments places on the stack.
SDValue BPUTargetLowering::LowerFormalArguments(
                                    SDValue Chain,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, BPU_CCallingConv);
...
bool BPUTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                MachineFunction &MF, bool isVarArg,
                                const SmallVectorImpl<ISD::OutputArg> &Outs,
                                LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  return CCInfo.CheckReturn(Outs, BPU_CRetConv);
}

SDValue BPUTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                       SmallVectorImpl<SDValue> &InVals) const {
  llvm_unreachable("Cannot lower call");
}

SDValue
BPUTargetLowering::LowerReturn(SDValue Chain,
  ...
  CCInfo.AnalyzeReturn(Outs, BPU_CRetConv);
  ...
  return DAG.getNode(BPUISD::Ret, dl, MVT::Other, RetOps);
}
...
SDValue
BPUTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported global address");
}

SDValue
BPUTargetLowering::LowerBlockAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported block address");
}

SDValue
BPUTargetLowering::LowerConstantPool(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported constant pool");
}

SDValue
BPUTargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  return SDValue();
}

SDValue
BPUTargetLowering::LowerShlParts(SDValue Op, SelectionDAG &DAG) const {
...
SDValue
BPUTargetLowering::LowerShrParts(SDValue Op, SelectionDAG &DAG,
                                   bool arith) const {
  assert(Op.getNumOperands() == 3 && "Not a long shift");
...
SDValue
BPUTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
